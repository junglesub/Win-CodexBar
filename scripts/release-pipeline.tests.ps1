...
Assert-True ($workflowText -match 'gh release list --limit 100 --json tagName --jq') 'rolling release existence uses a non-failing list query'
...
Assert-True ($workflowText -match 'select\(\.ref == "refs/tags/personal-latest"\)') 'tag probes select the exact ref'
